...
//	}
	...
	/*printf(
	 "l3->l3_getSlices = %d\nl3->l3info.slices=%d\nl3->l3info.setsperslice=%d\nl3->ngroups=%d\n",
	 l3_getSlices(l3), l3->l3info.slices, l3->l3info.setsperslice,
	 l3->ngroups);
	 printf("l3->groupsize=%d\n", l3->groupsize);
	 printf("l3->l3info.associativity=%d\n", l3->l3info.associativity);
	 */
...
int l3_repeatedprobecount_with_times(l3pp_t l3, int nrecords, uint16_t *results,  uint64_t *results_times_after, int slot) {
	assert(l3 != NULL);
	assert(results != NULL);
	...
	for (int i = 0; i < nrecords; i++, results += len) {
		if (missed) {
			...
			if (even)
				...
				l3_bprobecount(l3, results);
			results_times_after[i] = rdtscp64();
			even = !even;
...
int l3_repeatedprobecountbyline(l3pp_t l3, int nrecords, uint16_t *results,uint64_t *results_times_before, uint64_t *results_times_after, int slot, int set, int Xline) {
	assert(l3 != NULL);
	assert(results != NULL);
	...
	void * line = l3_getline(l3,set,Xline);
	void * lineCom = l3_getline(l3,set,11-Xline);

	void * FirstLine = l3_getline(l3,set,0);
	...
	for (int i = 0; i < nrecords; i++, results += 1) {


		if (missed) {
				results[0] = -1;
		} else {
			results_times_before[i] = rdtscp64();

			if (even){

					results[0] = memaccesstime(lineCom);

					probecount(FirstLine);
			}else{

					results[0] = memaccesstime(line);
					bprobecount(FirstLine);
			}
